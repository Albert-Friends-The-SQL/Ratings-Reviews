...
// const db = require('./index');
const userStream = fs.createWriteStream(__dirname + "/../usersTest.csv")
const productStream = fs.createWriteStream(__dirname + "/../productsTest.csv")
const reviewStream = fs.createWriteStream(__dirname + "/../reviewsTest.csv")
const lines = 100000;
...
const startWritingProduct = (writeStream, encoding, done) => {
  ...
  function writing() {
    ...
    do {
      ...
      let product = `${newProduct.productName}\n`
      //check if i === 0 so we would write and call `done`
      if (i === lines){
        // we are done so fire callback
